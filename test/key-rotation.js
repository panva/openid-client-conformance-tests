...
describe('Key Rotation', function () {
  ['rp-key-rotation-op-sign-key-native', 'rp-key-rotation-op-sign-key'].forEach((testId) => {
    it(`${testId} @code-config,@code-dynamic`, async function () {
      const { client } = await register(testId, { });
